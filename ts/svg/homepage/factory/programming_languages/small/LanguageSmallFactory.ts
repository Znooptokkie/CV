 ...
 */
class LanguageSmallContainerHTML
{
    private languageName: LanguageInterface[] = [];

    // Maakt de kleine SVG containers aan op basis van API-data
    public async createSmallSVGContainers(): Promise<SVGElement[] | null> 
    {
        const container = document.querySelector(".homepage-languages-small");
        ...
        for (const lang of data) 
        {
            ...
            svg.id = `${lang.name}-svg`;
            svg.classList.add("homepage-small-svg");
    ...
    public async getAPIData(): Promise<LanguageInterface[]>
    {
        // const fetcher = await Fetch.fetchJSON<LanguageInterface>("languages");
